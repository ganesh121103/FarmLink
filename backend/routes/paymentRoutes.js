...
router.post("/verify", verifyToken, async (req, res) => {
  try {
    ...
    // 🔔 TRIGGER: Notify Customer (Payment Successful) and Farmers
    setImmediate(async () => {
      try {
        ...
        if (customer) {
          await Notification.create({
            ...
            message: `Your payment of ₹${order.total} for Order #${orderIdStr} was successful.`,
            image: order.items?.[0]?.image || "",
            type: "Payment"
          });
          if (customer.fcmToken) {
            await sendPushNotification(customer.fcmToken, "Payment Successful! 💳", `Order #${orderIdStr} confirmed.`);
          }
        }
        ...
        for (const fId of farmerIds) {
          ...
          if (farmer) {
             await Notification.create({
               ...
               message: `You received a paid order (Order #${orderIdStr}). Check your dashboard!`,
               image: order.items.find(i => i.farmer === fId)?.image || "",
               type: "Payment"
             });
             if (farmer.fcmToken) {
               await sendPushNotification(farmer.fcmToken, "New Paid Order! 💸", `Order #${orderIdStr} received.`);
             }
          }
        }
      } catch (err) {
        console.error("❌ Failed to process payment notifications:", err.message);
      }
    });
