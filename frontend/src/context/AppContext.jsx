...
export const AppProvider = ({ children }) => {
    ...
    // ── Re-hydrate user from backend on refresh (restores profile image) ──
    useEffect(() => {
        const storedUser = (() => { try { return JSON.parse(localStorage.getItem('farmlink_user')); } catch { return null; } })();
        if (!storedUser?._id || !storedUser?.token) return;
        const refreshUser = async () => {
            try {
                const res = await fetch(
                    `http://localhost:5000/api/users/${storedUser._id}?role=${storedUser.role || 'customer'}`,
    ...
    const notificationFailCountRef = useRef(0);
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                ...
                notificationFailCountRef.current = 0; // reset on success
            }
        } catch (e) {
            notificationFailCountRef.current += 1;
            // Only show toast after 3 consecutive failures to avoid spam
            if (notificationFailCountRef.current === 3) {
                console.warn('[Notifications] Repeated fetch failures — backend may be down.');
            }
        }
    }, [user?.token]);
    ...
    useEffect(() => {
        if (user?.token) {
            ...
            setNotifications([]);
            setUnreadCount(0);
            notificationFailCountRef.current = 0;
    ...
    useEffect(() => {
        const handleAuthExpired = () => {
            // Only force-logout if the user actually has an active session
            const storedUser = (() => { try { return JSON.parse(localStorage.getItem('farmlink_user')); } catch { return null; } })();
            if (!storedUser?.token) return; // Ignore stale 401s from background polls after logout
            handleLogout();
