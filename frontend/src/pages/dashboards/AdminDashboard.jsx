...
import { Shield, Users, Package, Sprout, Sparkles, X, FileText, CheckCircle, Clock, BadgeCheck, Search, Calendar, AlertTriangle, CreditCard, IndianRupee, TrendingUp, RefreshCw, Loader2, Bell, Send } from 'lucide-react';
...
const AdminDashboard = ({ products, setProducts, farmers, orders, setOrders }) => {
    ...
    const handleDeleteProduct = async () => {
        ...
        try {
            ...
            console.log("Delete API call successful for:", productToDelete);
            setProducts(prev => prev.filter(p => p._id !== productToDelete));
            addToast('✅ Product removed successfully');
        } catch (err) {
            console.error("Delete product error:", err);
            addToast('❌ Failed to delete product: ' + (err.message || 'Unknown error'));
        } finally {
    ...
    return (
        <div className="pt-32 px-6 pb-24 max-w-7xl mx-auto">
            ...
            <div className="flex gap-4 mb-8 border-b border-stone-200 dark:border-slate-700 overflow-x-auto pb-1">
                ...
                <button onClick={() => setActiveTab('notifications')} className={`${tabClass('notifications')} flex items-center gap-2`}>
                                ...
                                <tr key={product._id} className={`group transition-colors cursor-pointer ${ isExpired ? 'bg-red-50/50 dark:bg-red-900/10' : isExpiringSoon ? 'bg-orange-50/50 dark:bg-orange-900/10' : 'hover:bg-stone-50 dark:hover:bg-slate-800/50'}`} onClick={() => setPreviewProduct(product)}>
                                    ...
                                    <td className="py-4">
                                        <button 
                                            ...
                                            className="text-xs py-1.5 px-3 font-bold border border-red-200 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                                        >
                                            {t('removeProduct')}
                                        </button>
                                    </td>
            ...
            {activeTab === 'notifications' && (
                <div className="animate-fade-in-up space-y-8">
                    ...
                    {/* System Test */}
                    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-stone-100 dark:border-slate-700 p-6 shadow-sm">
                        <h3 className="text-lg font-black text-black dark:text-white mb-1 flex items-center gap-2">
                            <Bell className="text-purple-500" size={20} /> System Diagnostics
                        </h3>
                        <p className="text-sm text-stone-500 mb-4">Create a test notification for yourself to verify the notification pipeline is working end-to-end.</p>
                        <button
                            id="btn-test-notification"
                            onClick={async () => {
                                setIsTestingNotif(true);
                                try {
                                    await apiCall('/notifications/test-self', 'GET');
                                    addToast('✅ Test notification created! Check the bell icon.');
                                } catch (e) {
                                    addToast('❌ Failed: ' + (e.message || 'Unknown error'));
                                } finally {
                                    setIsTestingNotif(false);
                                }
                            }}
                            disabled={isTestingNotif}
                            className="flex items-center gap-2 px-5 py-2.5 bg-purple-600 hover:bg-purple-700 active:scale-95 text-white font-bold rounded-xl transition-all disabled:opacity-60"
                        >
                            {isTestingNotif ? <><Loader2 size={16} className="animate-spin" /> Testing...</> : <><Bell size={16} /> Send Test Notification</>}
                        </button>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-stone-100 dark:border-slate-700 p-6 shadow-sm">
                        <h3 className="text-lg font-black text-black dark:text-white mb-1 flex items-center gap-2">
                            <Send className="text-green-500" size={20} /> Broadcast Notification
                        </h3>
                        <p className="text-sm text-stone-500 mb-5">Send a custom notification to any user by their MongoDB User ID.</p>
                        <div className="space-y-4 max-w-xl">
                            <div>
                                <label className="block text-xs font-bold text-stone-600 dark:text-stone-400 uppercase mb-1">Recipient User ID</label>
                                <input
                                    type="text"
                                    placeholder="MongoDB _id of the recipient"
                                    value={notifTarget}
                                    onChange={e => setNotifTarget(e.target.value)}
                                    className="w-full px-4 py-2.5 rounded-xl border border-stone-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm font-medium text-black dark:text-white outline-none focus:ring-2 focus:ring-purple-500"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-stone-600 dark:text-stone-400 uppercase mb-1">Title</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-stone-600 dark:text-stone-400 uppercase mb-1">Message</label>
                                <textarea
                                    rows={3}
                                    placeholder="Notification message body"
                                    value={notifMessage}
                                    onChange={e => setNotifMessage(e.target.value)}
                                    className="w-full px-4 py-2.5 rounded-xl border border-stone-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm font-medium text-black dark:text-white outline-none focus:ring-2 focus:ring-purple-500 resize-none"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-stone-600 dark:text-stone-400 uppercase mb-1">Type</label>
                                <select
                                    value={notifType}
                                    onChange={e => setNotifType(e.target.value)}
                                    className="w-full px-4 py-2.5 rounded-xl border border-stone-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm font-bold text-black dark:text-white outline-none focus:ring-2 focus:ring-purple-500"
                                >
                                    {['Admin','System','Order','Wishlist','NewArrival','PriceDrop','OutOfStock','Chat'].map(t => (
                            ...
                            <button
                                ...
                                disabled={isSendingNotif || !notifTarget.trim() || !notifTitle.trim() || !notifMessage.trim()}
                                onClick={async () => {
                                    setIsSendingNotif(true);
                                    try {
                                        await apiCall('/notifications', 'POST', {
                                            userId: notifTarget.trim(),
                                ...
                                className="flex items-center gap-2 px-6 py-2.5 bg-green-600 hover:bg-green-700 active:scale-95 text-white font-bold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {isSendingNotif ? <><Loader2 size={16} className="animate-spin" /> Sending...</> : <><Send size={16} /> Send Notification</>}
                            </button>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="p-5 sm:p-6 space-y-6">
                            ...
                            <div className="pt-4 border-t border-stone-100 dark:border-slate-700 flex justify-end gap-3 pb-1">
                                ...
                                <Button 
                                    ...
                                    onClick={() => {
                                        setProductToDelete(previewProduct._id);
                                        setDeleteModalOpen(true);
