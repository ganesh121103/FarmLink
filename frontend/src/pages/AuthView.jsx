...
const AuthView = ({ initialMode = 'login' }) => {
    ...
    const syncFirebaseUserWithBackend = async (firebaseUser, extraData = {}) => {
        ...
        try {
            ...
            const isConnectionError = err.message?.includes('Cannot connect to server') || 
            ...
            // Otherwise, it's a real backend validation/auth error, so rethrow it!
            throw err;
        }
    };
            ...
            if (mode === 'register') {
                ...
                if (!firebaseResult.error && firebaseResult.user) {
                    ...
                    try {
                        ...
                        if (isMountedRef.current) setErrors({ form: mongoErr.message || firebaseResult.error || 'Invalid credentials' });
                    }
                }
            }
        } catch (err) {
            if (isMountedRef.current) setErrors({ form: err.message || 'Authentication failed' });
        } finally {
    ...
    const confirmGoogleSignIn = async (selectedRole) => {
        ...
        try {
            ...
            if (error) {
                if (isMountedRef.current) setErrors({ form: error });
                sessionStorage.removeItem('farmlink_pending_role');
                return;
            }
            if (!firebaseUser) return; // User closed popup or redirect in progress
            ...
            if (isMountedRef.current) setErrors({ form: err.message || 'Google sign-in failed' });
        } finally {
